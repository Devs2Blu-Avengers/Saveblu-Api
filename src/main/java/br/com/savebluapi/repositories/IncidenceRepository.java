...
public interface IncidenceRepository extends JpaRepository<Incidence, Long> {
    ...
    @Query(nativeQuery = true, value = """
            SELECT i FROM Incidence i WHERE FUNCTION('ST_Distance_Sphere', 
            POINT(i.latitude = :latitude, i.longitude = :longitude), POINT(?1, ?2)) <= ?3
            """)
    ...
    @Query( nativeQuery = true, value = """
            SELECT i FROM Incident i ORDER BY FUNCTION('ST_Distance_Sphere', POINT(i.latitude = :latitude
            , i.longitude = :longitude), POINT(?1, ?2))
            """)
